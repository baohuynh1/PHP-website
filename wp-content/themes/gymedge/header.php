...
$light_logo_width = GymEdge::$options['logo_light']['width'];
$light_logo_height = GymEdge::$options['logo_light']['height'];

$dark_logo_dimensions = (!empty($dark_logo_width) && !empty($dark_logo_height)) ? 'width="'.$dark_logo_width.'" height="'.$dark_logo_height.'"' : 'width="153" height="42"';
$light_logo_dimensions = (!empty($light_logo_width) && !empty($light_logo_height)) ? 'width="'.$light_logo_width.'" height="'.$light_logo_height.'"' : 'width="153" height="42"';
...
<body <?php body_class(); ?>>
	...
	<?php
	// Preloader
	if ( GymEdge::$preloader == 1 || GymEdge::$preloader == 'on' ){
		if ( !empty( GymEdge::$options['preloader_image']['url'] ) ) {
			$preloader_img = GymEdge::$options['preloader_image']['url'];
		}
		else {
			$preloader_img = GYMEDGE_IMG_URL . 'preloader.gif';
		}
		echo '<div id="preloader" style="background-image:url(' . esc_url( $preloader_img ) . ');"></div>';
	}
	?>
	<div id="page" class="site">
		<a class="skip-link screen-reader-text" href="#main"><?php esc_html_e( 'Skip to content', 'gymedge' ); ?></a>
					...
					<div class="<?php echo esc_attr( $gym_logo_class );?>">
						<div class="site-branding">
							<a class="dark-logo" href="<?php echo esc_url( home_url( '/' ) );?>">
								<img <?php echo $dark_logo_dimensions; ?> src="<?php echo esc_url( $gym_dark_logo );?>" alt="<?php esc_attr( bloginfo( 'name' ) ) ;?>">
							</a>
							<a class="light-logo" href="<?php echo esc_url( home_url( '/' ) );?>">
								<img <?php echo $light_logo_dimensions; ?> src="<?php echo esc_url( $gym_light_logo );?>" alt="<?php esc_attr( bloginfo( 'name' ) ) ;?>">
							</a>
						</div>
					</div>
		...
		<!-- <div id="meanmenu"></div> -->
		<?php get_template_part('template-parts/header', 'offscreen'); ?>
