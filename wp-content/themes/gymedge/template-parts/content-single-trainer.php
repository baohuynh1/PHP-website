...
$gym_education   = get_post_meta( $post->ID, 'gym_trainer_education', true );
...
$trainer_text = apply_filters( 'gymedge_trainer_text', [
	'Education',
	'Experience',
	'Age',
	'Weight',
        ...
        <div class="<?php echo esc_attr( $gym_trainer_img_class ); ?>">
            <div class="trainer-detail-image">
                ...
                <div class="trainer-info">
					<?php echo empty( $gym_education ) ? '' : '<p><span>' . esc_html__( $trainer_text[0], 'gymedge' ) . ':</span>' . esc_html( $gym_education ) . '</p>'; ?>
					<?php echo empty( $gym_experience ) ? '' : '<p><span>' . esc_html__( $trainer_text[1], 'gymedge' ) . ':</span>' . esc_html( $gym_experience ) . '</p>'; ?>
					<?php echo empty( $gym_age ) ? '' : '<p><span>' . esc_html__( $trainer_text[2], 'gymedge' ) . ':</span>' . esc_html( $gym_age ) . '</p>'; ?>
					<?php echo empty( $gym_weight ) ? '' : '<p><span>' . esc_html__( $trainer_text[3], 'gymedge' ) . ':</span>' . esc_html( $gym_weight ) . '</p>'; ?>
					<?php echo empty( $gym_email ) ? '' : '<p><span>' . esc_html__( $trainer_text[4], 'gymedge' ) . ':</span><a href="mailto:' . esc_attr( $gym_email ) . '">' . esc_html( $gym_email ) . '</a></p>'; ?>
					<?php echo empty( $gym_phone ) ? '' : '<p><span>' . esc_html__( $trainer_text[5], 'gymedge' ) . ':</span><a href="mailto:' . esc_attr( $gym_phone ) . '">' . esc_html( $gym_phone ) . '</a></p>'; ?>
        ...
        <div class="<?php echo esc_attr( $gym_trainer_details_class ); ?>">
            <div class="trainer-detail-content">
                <div class="detail-heading">
                    ...
                    <h3 class="title"><?php esc_html_e( $trainer_text[6] . ':', 'gymedge' ); ?></h3>
					<?php the_content(); ?>
                </div>
				<?php if ( ! empty( $gym_skills ) ): ?>
                    <div class="trainer-skills">
                        <h3><?php esc_html_e( $trainer_text[7] . ':', 'gymedge' ); ?></h3>
                        <div class="skill">
