...
abstract class Custom_Widget_Base extends Widget_Base {
	...
	public function __construct( $data = [], $args = null ) {
		$this->rt_category = GYMEDGE_CORE_THEME_PREFIX . '-widgets'; // Category /@dev
	...
	function rt_get_all_image_sizes() {
		...
		foreach ( $_wp_additional_image_sizes as $index => $item ) {
			$image_sizes[ $index ] = __( ucwords( $index . ' - ' . $item['width'] . 'x' . $item['height'] ), 'greenova_core' );
		}
		$image_sizes['full'] = __( "Full Size", 'greenova_core' );
