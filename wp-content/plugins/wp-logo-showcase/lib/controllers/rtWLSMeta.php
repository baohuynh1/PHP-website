...
if ( ! class_exists( 'rtWLSMeta' ) ):
	...
	class rtWLSMeta {
		...
		public function arrange_wlshowcase_columns( $columns ) {
			$column_thumbnail = array( 'wls_logo_thumb' => esc_html__( 'Logo Image', 'wp-logo-showcase' ) );
			$column_url       = array( 'wls_logo_client_url' => esc_html__( 'Client Website Url', 'wp-logo-showcase' ) );
		...
		function wls_logo_image_box() {
			...
			add_meta_box( 'postimagediv', esc_html__( 'Logo Image', "wp-logo-showcase" ), 'post_thumbnail_meta_box', $rtWLS->post_type, 'normal',
				'high' );
		}
