  ...
  var rtApp = {
    ...
    conditionalField: function conditionalField() {
      ...
      function showHideMeta() {
        var menualSnipet = $("#_rtrs_disable_snippet_generator_holder");
        var richSnippet = menualSnipet.find('#rtrs-disable_rich_snippet');
        var isdisable = richSnippet.prop('checked');
        ...
        if (isdisable) {
          $('#_rtrs_rich_snippet_cat_holder').addClass('rtrs-hidden');
        } else {
          $('#_rtrs_rich_snippet_cat_holder').removeClass('rtrs-hidden');
        }

        $('.rtrs-schema-field').hide();

        if (rich_snippet_cat && !isdisable) {
    ...
    metaImage: function metaImage() {
      ...
      $(document).on('click', '.rtrs-file-remove', function (e) {
        ...
        if (confirm(rtrs.sure_txt)) {
          if ($(this).parents(".rtrs-preview-video").length) {
            $(this).parents(".rtrs-preview-video").remove();
          }
